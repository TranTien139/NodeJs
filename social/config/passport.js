...
var FACEBOOK_APP_ID = "1809357332636523"
var FACEBOOK_APP_SECRET = "854746d2d2a894ee0d579c251e749d2f";
       ...
       process.nextTick(function() {
            User.findOne({ 'user.email' :  email }, function(err, user) {
                ...
                if (!user.verifyPassword(password))
                    return done(null, false, req.flash('error', 'Lỗi nhập mật khẩu'));
               else
        ...
        process.nextTick(function() {
            ...
            if (!req.user) {
                User.findOne({ 'user.email' :  email }, function(err, user) {
            	    if (err){ return done(err);}
                    if (user) {
                        return done(null, false, req.flash('signuperror', 'Tài khoản đã tồn tại'));
                    } else {
	...
	passport.use(new FacebookStrategy({
  		...
  		function(req, accessToken, refreshToken, profile, done) {
    		// asynchronous verification, for effect...
    			...
    			process.nextTick(function () {
            			if (!req.user) {
                          var face=  profile.emails? profile.emails[0].value:'test@facebook.vn';
 					User.findOne({ 'user.email' :  face }, function(err, user) {
            	    				if (err){ return done(err);}
                    				if (user) {
                        				return done(null, user);
                    				} else {
							...
							newUser.user.username    = profile.displayName;
                        				newUser.user.email    =  profile.emails? profile.emails[0].value:'test@facebook.vn';
							newUser.user.name	= ''
							newUser.user.address	= ''
